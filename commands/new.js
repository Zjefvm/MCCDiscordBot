...
exports.use = async (client, message, nothing, args, command) => {
    ...
    if (args[0]) {
        ...
        message.guild.createChannel(`ticket-${data.id}`).then(async c => {
            ...
            await c.send(new Discord.RichEmbed().addField('Onderwerp', `${reason}`).addField('Leg uit', "Leg goed uit wat er aan de hand is, om een snellere reactie te krijgen!" ).setDescription(`Bedankt voor het maken van het ticket!.\nDe staffleden zulllen je zo snel mogelijk helpen!`).setColor(Config.ticketcolor))
        })
        data.id++;
